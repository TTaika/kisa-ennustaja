    ...
    render(body, close) {
      body.innerHTML = `
        <form id="task-form">
          ...
          <div class="row">
            <label>Suorituspaikkoja <input name="slots" type="number" min="1" value="${seed.slots ?? 1}" ${seed.unlimitedSlots ? "disabled" : ""}></label>
          ...
          <div class="row">
            <label>Nopein (min) <input name="fastestTaskMin" type="number" min="0" value="${seed.fastestTaskMin ?? 10}"></label>
            <label>Hitain (min) <input name="slowestTaskMin" type="number" min="0" value="${seed.slowestTaskMin ?? 20}"></label>
            <label>Häröily-lisä (min) <input name="haroilyMin" type="number" min="0" value="${seed.haroilyMin ?? 0}"></label>
          </div>
      ...
      form.addEventListener("submit", (e) => {
        e.preventDefault();
        const values = {
          name: field("name").value.trim() || "Nimetön tehtävä",
          slots: Math.max(1, Math.round(+field("slots").value || 1)),
          unlimitedSlots: unlimitedI.checked,
          fastestTaskMin: Math.max(0, +field("fastestTaskMin").value || 0),
          slowestTaskMin: Math.max(0, +field("slowestTaskMin").value || 0),
          haroilyMin: Math.max(0, +field("haroilyMin").value || 0)
        };
        update(st => {
